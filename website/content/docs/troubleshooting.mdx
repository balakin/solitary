...
| `orphaned`      | The machine is here, but the definition it came from is gone.      |
...
An `orphaned` machine is what a deleted or renamed cell directory leaves behind, holding its disk with nothing left to name it. `solitary rm --orphans` discards them all; `solitary init <name>` gives one a definition again. `doctor` reports them too.
...
`up, no recent handshake` in the dashboard, and a cell that reaches nothing — the kill switch doing its job on a tunnel that is no longer carrying anything. The usual cause is the peer's address moving out from under a tunnel that resolved it once at boot.

The machine watches for this and re-points the tunnel within a minute, and `solitary up` brings up a tunnel that has stopped answering. A cell whose machine has not booted since that watchdog was added does not have it yet, so if it keeps happening, restart the cell once:

```sh
solitary down <cell> && solitary up <cell>
```

What it does is described under [staying connected](/docs/guides-vpn#staying-connected).
