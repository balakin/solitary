...
## A change to `vm` that did not take

Lima cannot apply machine settings to a running machine, so `up` warns rather than silently ignoring them:

```text
Warning: the vm settings for "claude" changed since its machine started.
         The running cell still uses the old ones. To apply the change:
