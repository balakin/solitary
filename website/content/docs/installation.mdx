...
The script refuses to run while a machine is still on the host rather than orphaning it: once the binary is gone, the tool that knows how to remove one is gone too. `SOLITARY_INSTALL_DIR` points it at an install somewhere unusual, the same as during an install.
