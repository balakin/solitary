...
A domain covers its subdomains, so `github.com` reaches `api.github.com`. It does not cover a different domain the same site happens to use, so `objects.githubusercontent.com` has to be listed too. IPv4 addresses and CIDR blocks are used as written; IPv6 is refused when the cell is read, because a cell's machine has no route to reach it through.
