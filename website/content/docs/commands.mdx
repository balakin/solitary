...
It is idempotent: it creates the cell if absent, boots it if stopped, and attaches if it is already running. It also replaces the container when the image or the secrets changed, so editing `cell.yaml` and running `up` again is all a change ever takes.

| Flag | Meaning |
| --- | --- |
| `--detach` | Start the cell without attaching a shell. |
| `--as <name>` | The name to give a cell created from an image reference. |

Given an **image reference** rather than a known cell name, `up` creates a cell from it with default settings:

```sh
solitary up docker.io/library/ubuntu:24.04
```

If a cell of the derived name already exists with a different image, the command fails and asks for `--as`.
