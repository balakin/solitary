...
`wg-quick` resolves the peer's name once, when it brings the tunnel up, and pins the address it got. A provider that balances one name across servers — or retires the server this cell dialled — leaves the interface up and handshaking with an address that answers nothing. Inside the cell that looks exactly like the network being down, and nothing about it recovers on its own.

So the machine keeps a watchdog on the tunnel, once a minute:

- It sets a **persistent keepalive** on the peer, so the tunnel handshakes about every two minutes whether or not the cell is using it. That holds the machine's NAT mapping open, and it makes the age of the last handshake mean something: silence is a fault rather than an idle cell.
- After three minutes without a handshake it **re-resolves the peer's name** — through the cell's own resolver, which is also what allows the answer through the firewall — and re-points the tunnel at whatever the name means now.
- If the interface is gone entirely, which is what `wg-quick` leaves behind when it fails, it **starts it again**.

The keepalive is set on the running interface, not written into your file: the `.conf` stays exactly as the provider gave it. The machine logs what it does to the journal under the `solitary` tag.

`solitary up` does the same check on the way past. A tunnel whose configuration has not changed is normally left alone, connections and all — but one that has stopped answering is brought up again:

```text
The tunnel to de-ber.prod.surfshark.com has stopped answering; bringing it up again...
```

A machine created before this existed picks up the watchdog at its next boot: `solitary down <cell> && solitary up <cell>`.
